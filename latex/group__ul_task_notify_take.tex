...
ul\+Task\+Notify\+Take() is intended for use when a task notification is used as a faster and lighter weight binary or counting semaphore alternative. Actual Free\+R\+T\+O\+S semaphores are taken using the x\+Semaphore\+Take() A\+P\+I function, the equivalent action that instead uses a task notification is ul\+Task\+Notify\+Take().
...
A task can use ul\+Task\+Notify\+Take() to \mbox{[}optionally\mbox{]} block to wait for a the task\textquotesingle{}s notification value to be non-\/zero. The task does not consume any C\+P\+U time while it is in the Blocked state.
